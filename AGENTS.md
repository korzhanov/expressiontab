...
- **Dev**: `bun run dev` (`scripts/watch-build.mjs`) → F5 на new-tab; CursorBrowser: `bun run preview` → http://localhost:4173/newtab/ (mock chrome.*). Bubble-режим: `d3-force` поле (`BubbleField`); lined — VirtualScroll как раньше.
